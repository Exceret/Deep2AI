...
class SigmoidActivator(object):
    ...
    def forward(self: np.ndarray, weighted_input: np.ndarray) -> np.ndarray:
        """_summary_

        Args:
            self (np.ndarray): _description_
            weighted_input (np.ndarray): _description_
    ...
    def backward(self: np.ndarray, output: np.ndarray) -> np.ndarray:
        """_summary_
...
class LstmLayer(object):
    ...
    def calc_delta_k(self, k):
        ...
        delta_i = (
            delta_k * og * (1 - tanh_c * tanh_c) * ct * self.gate_activator.backward(ig)
        )
        delta_ct = (
            delta_k
            * og
...
def gradient_check():
    ...
    for i in range(lstm.Wfh.shape[0]):
        for j in range(lstm.Wfh.shape[1]):
            ...
            lstm.forward(x[0])
            lstm.forward(x[1])
            err1 = lstm.h_list[-1].sum()  # * error_function(lstm.h_list[-1])
            lstm.Wfh[i, j] -= 2 * epsilon
            lstm.reset_state()
            lstm.forward(x[0])
            lstm.forward(x[1])
            err2 = lstm.h_list[-1].sum()  # * error_function(lstm.h_list[-1])
            expect_grad = (err1 - err2) / (2 * epsilon)
            lstm.Wfh[i, j] += epsilon
            print(
                f"weights({i},{j}): expected - actural {expect_grad} - {lstm.Wfh_grad[i, j]}"
            )
    return lstm
