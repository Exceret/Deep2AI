...
class RecurrentLayer(object):
    def __init__(
        ...
        self.times: int = 0  # 当前时刻初始化为t0
        self.state_list: List[np.ndarray] = []  # 保存各个时刻的state
        self.state_list.append(np.zeros((state_width, 1)))  # 初始化s0
        self.U: np.ndarray = np.random.uniform(-1e-4, 1e-4, (state_width, input_width))  # 初始化U
        self.W: np.ndarray = np.random.uniform(-1e-4, 1e-4, (state_width, state_width))  # 初始化W
        self.gradient_list: List[np.ndarray] = []  # 保存各个时刻的权重梯度
        self.delta_list: List[np.ndarray] = []
        self.gradient: np.ndarray = np.zeros((state_width, state_width))
    ...
    def backward(self, sensitivity_array: np.ndarray, activator: Union[ReluActivator, IdentityActivator]) -> None:
        """
        实现BPTT算法
        """
    ...
    def calc_delta(self, sensitivity_array: np.ndarray, activator: Union[ReluActivator, IdentityActivator]) -> None:
        """_summary_
    ...
    def calc_delta_k(self, k: int, activator: Union[ReluActivator, IdentityActivator]) -> None:
        """
    ...
    def calc_gradient_t(self, t: int) -> None:
        """
        计算每个时刻t权重的梯度
        """
...
def data_set() -> Tuple[List[np.ndarray], np.ndarray]:
    """_summary_
